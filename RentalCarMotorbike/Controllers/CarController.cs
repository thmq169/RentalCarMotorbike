...
namespace RentalCarMotorbike.Controllers
{
    ...
    public class CarController : Controller
    {
        public ActionResult Index()
        {
            ...
            {
                var cars = db.Vehicle.Where(v => v.VehicleStatus == "Available" && v.VehicleModel == "Car");
        ...
        public ActionResult Rent(string id,FormCollection collection)
        {
            try {
                using (RentalVehicalDB db = new RentalVehicalDB())
                {
                    ...
                    return Json(new { code = 0,message = "Success", days = Days, idRent = rentID }, JsonRequestBehavior.AllowGet); ;
                } }
            catch (Exception ex)
            {
